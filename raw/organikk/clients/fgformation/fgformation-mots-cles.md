...
title: "RECHERCHE MOTS-CLÉS — Qualiopi / FG Formation (depuis les 14 calls)"
aliases: []
tags: []
created: 2026-06-08
updated: 2026-06-08
...
---
client: FG Formation
type: Recherche mots-clés (skill seo-recherche-mots-cles)
source: [[fgformation-personas-problematiques]] (14 calls réels)
date: 2026-06-06
relié: [[fgformation]]
---

# RECHERCHE MOTS-CLÉS — Qualiopi / FG Formation (depuis les 14 calls)

> **En résumé.** ~130 requêtes générées depuis la matière terrain, qualifiées par intention ([[concepts/know-simple-know-do|Know-Simple / Know / Do]]) et étage de funnel, organisées par les 9 personas. Aucun volume inventé : tout est `[À SOURCER]` (à brancher sur GSC FG Formation + Ahrefs/[[entities/semrush|Semrush]]). La logique Tim est appliquée : on écarte les têtes génériques (« accompagnement Qualiopi », déjà top 3, plafonné) et on vise les **problématiques** et les **[[concepts/mots-cles-actionnels|requêtes décisionnelles longue traîne]]** où personne ne se positionne. Colonne **Note** = angle de page / point de conversion.
>
> **Difficulté** = proxy déclaratif (nb de mots, présence de têtes génériques, type de SERP attendu), pas une donnée d'outil. À confirmer.
> **Axe** : `F` = formateur indépendant · `OF` = organisme de formation · `T` = transverse. (Rappel : séparer les deux arbres sur le site.)
...
- **~130 mots-clés** générés depuis les 14 calls.
- Répartition par intention (estimée) : **~55 Do / décision · ~65 Know · ~10 Know-Simple**.
- Le gisement est dans les **Do longue traîne faible difficulté** (P1, P4, P5, P7, P8) : requêtes que personne ne travaille, à intention de devis, où viser la position 1 est réaliste.
- Les **têtes génériques sont écartées** (déjà acquises et plafonnées), conformément à la consigne « tuer les pages généralistes ».

### Top 12 à prioriser (Do, faible difficulté, fort signal terrain)

1. `qualiopi pour travailler en direct avec les entreprises` (P1)
2. `préparer son audit de surveillance qualiopi` (P5)
3. `audit blanc qualiopi` (P5)
4. `mon nda a été refusé que faire` (P8)
5. `qualiopi pour un centre de formation interne` (P4)
6. `redevenir qualiopi après l'avoir perdu` (P4)
7. `qualiopi pour une association qui forme ses membres` (P7)
8. `monter une formation ia finançable cpf` (P6)
9. `passer en direct quand on est formateur sous-traitant` (P1)
10. `transférer son nda d'auto-entrepreneur vers une société` (P8)
11. `qualiopi obligatoire pour les formations d'avocats` (P7)
12. `diagnostic qualiopi avant de se lancer` (P7, + lead magnet quiz)

---

## Prochaines étapes (workflow)

1. **Sourcer les volumes** : brancher la GSC de FG Formation (les requêtes où le site est déjà en page 2-3 = quick wins) + Ahrefs/Semrush sur la liste. Remplacer les `[À SOURCER]`.
2. `seo-clustering-mots-cles` : regrouper par partage de SERP → 1 cluster = 1 page (certaines lignes ci-dessus fusionnent), pour éviter la [[concepts/triade-serp|cannibalisation]].
3. `seo-mots-cles-decisionnels` : valider la shortlist décisionnelle (intention de devis réelle).
