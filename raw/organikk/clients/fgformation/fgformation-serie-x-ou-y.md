...
- **2026-07-24 (correction du gabarit HTML)** : première repasse faite uniquement sur les `.md` sources dans `pages/` — Tim a relu le HTML final dans `pages/html/` (le vrai livrable, celui posé dans le Drive en Google Doc) et il contenait encore le texte d'origine, intact. Les 7 fichiers `pages/html/duel-*.html` ont été corrigés un par un avec les mêmes changements que la repasse markdown (resume-box, H2, sommaire, corps, FAQ). Leçon : sur cette série, `.md` et `.html` sont deux copies indépendantes du même texte, aucune ne régénère l'autre — toute correction de fond doit être appliquée aux deux, sans quoi le HTML transmis au client reste l'ancienne version.
- **2026-07-24 (repasse registre oral)** : Tim a rejeté une nouvelle fois le style, cette fois en visant le registre plutôt que les patterns IA ponctuels : « tu dois tout réécrire comme on parle », en citant un extrait encore trop écrit (phrases longues, subordonnées empilées, connecteurs de dissertation). Repasse complète des 7 pages (`.md` + `.html`) phrase par phrase : découpage des phrases longues en phrases courtes sujet-verbe, "on" à la place de "nous" dans le corps, tournures orales (« Ça, c'est... », « Sauf que... », « ça se joue sur... » à la place de « il se joue sur »), suppression des tournures de dissertation (« il convient de », « ce qui ressort de », constructions en incise chargées). Grille stable, tableaux, FAQ, sources, verbatims, chiffres : inchangés. Les 7 fichiers HTML re-vérifiés (`<a>` ouvrants/fermants comptés égaux sur les 7) après plusieurs balises cassées en cours d'édition, corrigées à chaque fois avant de continuer.
