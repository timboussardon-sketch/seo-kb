---
title: "Skill bootcamp Session 3 — maillage-systeme (skill complet)"
bootcamp: 4
semaine: 3
type: skill-distribuable
usage: "Bundle Drive S3 (déjà dans le pack des 9 #4 — re-distribué pour que le dossier audit soit autonome)"
related:
...
# Skill `maillage-systeme` — Session 3 (skill complet)

Passe **structurelle** de la Phase 5 du workflow audit (5B). Raisonne sur l'architecture éditoriale (piliers, hub/satellite, ancres, orphelines), **sans dépendre de la GSC**. Se chaîne avec `maillage-interne-gsc` (passe data, 5C).

> ℹ️ Ce skill est déjà dans le **pack des 9 de la S1 (#4)** — tu l'as normalement déjà. On le re-met dans le dossier Drive S3 pour que le bundle audit soit autonome et que tu vérifies que tu as bien la **version complète** (certains avaient installé une version courte en S1). Si ton `~/.claude/skills/maillage-systeme/SKILL.md` est plus court que le bloc ci-dessous, remplace-le.

## Procédure d'install / vérification

1. Dossier `~/.claude/skills/maillage-systeme/`
2. `SKILL.md` = tout le bloc entre les `=====` ci-dessous (écrase l'ancien si version courte)
...
- Contenu **identique au skill canonique** `~/.claude/skills/maillage-systeme/SKILL.md` (252 lignes), reproduit verbatim — pas de section privée à purger ici (contrairement à `indexation-check`).
- **Déjà dans le pack des 9 (#4)** : ce n'est PAS un nouveau skill à installer pour ceux qui ont la version complète. Le re-bundle S3 sert (a) à rendre le dossier Drive audit autonome, (b) à rattraper ceux qui avaient mis la version courte en S1 (problème connu, cf. [[observations-whatsapp-bootcamp]] §2). Le message week-end doit le présenter comme « vérification », pas « nouvelle install », pour ne pas affoler.
- **À ne pas confondre** dans le message groupe : `maillage-systeme` = pack #4 (déjà là) ≠ `maillage-interne-gsc` = hors pack, vraie nouvelle install pour la passe data de la Phase 5. Les deux servent la Phase 5, l'un structurel l'autre data.
- Si tu mets à jour le skill canonique, régénère ce fichier.
