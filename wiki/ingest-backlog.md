...
>
> **Lot W31 voté le 2026-07-24** ([[revue-hebdo/2026-W30]] point 3) : **FG-Formation** — `fgformation-patterns-requetes` + `fgformation-modele-corpus` (dormants depuis le 07-14) + `fgformation-serie-x-ou-y.md` + les 7 pages du duel « X ou Y » produites cette semaine, plus `notes/fg-formation/audit-blanc.md` et les 5 transcripts `audit-blanc-*` (ferme [[contradictions#C-005]] au passage). **Le vote Victoria Garden est retiré, deuxième non-exécution consécutive** — même sort que Leexi en W29 : le dossier reste un chantier vivant (recherche mots-clés business livrée le 07-23) mais ce n'est jamais son backlog `raw/` que le travail réel tire, c'est la production de nouvelles analyses. Le lot ne se revote plus sur Victoria Garden par défaut ; il repart si un ingest de ses 11 fichiers P1 est explicitement demandé.
...
> **Sweep 2026-07-20** : 379 fichiers `raw/` scannés (+6 vs sweep précédent), backlog à **245** (P1:127 P2:35 P3:83). Semaine calme côté capture, mais les 6 fichiers neufs tombent tous sur des chantiers vivants — aucun bruit. **Sorties (0)** : rien n'est sorti du backlog cette semaine. `raw/transcripts/call-15-baptiste-yalp.md` (2026-07-17) a été **ingéré à chaud** le 2026-07-16 ([[sources/2026-07-16-call-15-baptiste-yalp]], + [[entities/baptiste-yalp]] et [[entities/yalp]]) sans jamais entrer au backlog — même trajectoire que le call 14 Vincent au sweep précédent : les calls frais se traitent le jour même, ils ne s'accumulent pas. Le paper `papers/google-quality-raters-guidelines-2026.md`, retouché le 07-17, reste couvert par [[sources/2026-04-13-google-quality-raters-guidelines-2026]] ; `notes/tim-my-voice.md` retouché le 07-17 reste un skip documenté. **Entrées (4 P1 + 1 P3)** : **FG-Formation redémarre** (+2, `fgformation-patterns-requetes` 416 lignes de reverse engineering des modèles de requêtes + `fgformation-modele-corpus`, produits le 2026-07-14) — le bloc de 40 fichiers dormant depuis le 2026-06-13 passe à 42 et redevient un chantier vivant, pas une archive ; **Victoria Garden** +1 (`corpus-modele/motif-seminaire-diagnostic-2026-07-17`, diagnostic motif séminaire adossé à l'export GSC du 06-11 + scraping des 4 pages existantes) qui porte le lot W30 à 11 fichiers ; **Alexia** +1 (`2026-07-17-call-3-resume`, passage terminal + Obsidian, migration Shopify) qui porte le dossier à 4 calls. En P3, `organikk/prospection/journal-prospection.md` — fichier **synchronisé automatiquement depuis Supabase toutes les heures** (473 prospects, 131 invitations, 13 acceptées), explicitement non éditable à la main : c'est un tracker vivant, pas une source. Même traitement que `reddit-playbook/Journal.md` — sa doctrine est déjà capturée dans [[methodes/farming-prospection]], il n'appelle pas de `wiki/sources/` mais une fiche preuve quand la data de conversion sera constituée. **Candidat skip** à trancher. Lecture du sweep : la doctrine posée en W29 (« un ingest se rattache à une échéance datée ou à un chantier vivant, jamais à un vote hebdomadaire ») se vérifie — les trois dossiers qui bougent cette semaine (VG, FG, Alexia) sont exactement les trois clients à chantier ouvert, et le backlog n'a pris aucun fichier orphelin. Backlog > 80 → on liste les 15 premiers par tier (oldest-first) et on agrège le reste par groupe.
...
## Priorité 1 — Données terrain propriétaires non capturées (le moat qui fuit) — 127
...
- **Alexia — dossier client** (2026-06-10 → 07-17, 4 fichiers) : `clients/alexia/2026-06-10-call-cadrage-adrien`, `-resume-call-envoye`, `2026-06-24-call-1-resume` + **`2026-07-17-call-3-resume` neuf** (passage au terminal + Obsidian, workflow mots-clés, migration Shopify) — accompagnement 1-1 vivant, calls archivés en raw (actions `client` au [[log]]) mais sans `wiki/sources/`. Le call 3 documente la transposition du setup de Tim chez un client : matière directe pour [[syntheses/workflow-complet-consultant-seo-ia]].
- **FG-Formation — dossier client complet** (2026-06-13 → 07-14, 42 fichiers) : `clients/fgformation/calls/` (32 — audits blancs ×5, coachings ×6, R1 ×3, RDV ×17, `SYNTHESE-appels-anonymisee`) + 10 livrables (`fgformation` maître, `-clusters`, `-gsc-quickwins`, `-modeles-pseo`, `-mots-cles`, `-mots-cles-decisionnels`, `-mots-cles-recherche-exhaustive`, `-personas-problematiques` + **2 neufs du 2026-07-14** : `-patterns-requetes` 416 lignes de reverse engineering des modèles de requêtes, `-modele-corpus` doctrine corpus-avant-pages appliquée à FG). **Le plus gros bloc terrain frais de tout le backlog** : 32 transcripts d'appels réels = matière brute pour [[concepts/avatar-freelance-sans-systeme]] et le discours commercial, + livrables pSEO produits. Aucun équivalent wiki. **Le dossier redémarre** : dormant depuis le 2026-06-13, il reprend le 07-14 avec le même pattern corpus que Victoria Garden et Golfiller — troisième client sur le même modèle de production, c'est le signal d'une doctrine transposable à ficher.
...
## Priorité 3 — À traiter selon valeur — 83 (+ drive-accompagnement parké)
...
**+ 68 autres P3, par groupe :**

- `raw/organikk/prospection/journal-prospection.md` (2026-07-13, `source_type: client-note`) — file LinkedIn synchronisée depuis la table Supabase `prospection_pipeline` toutes les heures, non éditable à la main (473 prospects, 131 invitations parties, 13 acceptées au 2026-07-16). Tracker vivant, pas une source : sa doctrine est déjà dans [[methodes/farming-prospection]], qui le cite comme le versant hunting de la boucle. À ne pas ingérer en `wiki/sources/` — appelle une **fiche preuve** quand la data de conversion sera constituée. **Candidat skip**, à trancher par Tim.
