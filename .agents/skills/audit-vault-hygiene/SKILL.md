...
- `.git/`, `.obsidian/`, `.obsidian.broken/`, `.Codex/`, `.github/`, `node_modules/`
...
fd -e md -E '.git' -E '.obsidian*' -E '.Codex' -E '.github' -E '_archive' -E 'mockups' -E 'node_modules' . > /tmp/vault-files.txt 2>/dev/null \
  || find . -name "*.md" -not -path "./.git/*" -not -path "./.obsidian*/*" -not -path "./.Codex/*" -not -path "./.github/*" -not -path "*/_archive/*" -not -path "./mockups/*" -not -path "./node_modules/*" > /tmp/vault-files.txt
...
- `wiki/index.md`, `wiki/log.md`, `MEMORY.md`, `AGENTS.md`, `AGENTS.md`, `README.md`
