...
## Questions & propositions — 2026-08-07-v2 (groupées pour revue hebdo)

- **Q-2026-08-07-v2-1 (non urgent, revue hebdo)** — Absence de concept doctrine wiki sur le **régime légal des agents tiers en droit européen** (Data Act, DSA, RGPD appliqué à un agent qui accède à un site pour le compte d'un utilisateur identifié). La décision 9e Circuit du 4 août 2026 sur Comet-Amazon crée une doctrine américaine « user access, not agent access » mais aucun équivalent européen n'est cartographié dans le wiki. Proposition : créer `wiki/concepts/regime-legal-agent-tiers-eu.md` quand une première décision européenne tombe (CJUE, Cour de cassation FR, BGH allemand), ou ouvrir une page hypothèse dans `wiki/hypotheses.md` avant.
- **Q-2026-08-07-v2-2 (non urgent, revue hebdo)** — Suggestion durcissement discipline volatilité SERP : demander à Tim si l'agent doit toujours WebFetch un tracker primaire (Advanced Web Ranking, Semrush Sensor public, Sistrix Toolbox, Mozcast) avant de citer un pic de volatilité, ou si une reprise blog SEO à trust >=0.55 suffit avec flag « single-source direction ». Ce run a retenu la seconde règle (SEO Vendor + confidence 0.75) faute de disponibilité WebFetch tracker primaire dans la fenêtre.
- **Q-2026-08-07-v2-3 (non urgent, revue hebdo)** — Le mot « bataille » a été détecté à la relecture dans « bataille juridique » (info du jour B1). Métaphore banie. Corrigé. À ajouter à la liste des mots piège récurrents en édition legal (au même titre que « rail » sur édition 07-17). Le contexte juridique attire mécaniquement le lexique combatif ; à surveiller sur chaque édition traitant d'une décision de justice.


## Questions issues du run 2026-08-08

- **Q-2026-08-08-1 (non urgent, revue hebdo)** — L'expérience Otterly.ai capybara est un cas limite précieux (test contrôlé sur 4 mois avec chiffres opposés Google/Bing sur même corpus), mais reste single-source vendor GEO. Le pattern général (Google mass deindex April 2026) est corroboré par 3 sources indépendantes non-Otterly (Cicero Alexis Dollé + Barry Schwartz Seroundtable + Digital Applied/Glenn Gabe/Krishna Madhavan). Ma lecture retenue : présenter le fait single-source Otterly comme direction avec caveat explicite dans le corps, cadrer par le pattern général corroboré, et créer P-2026-08-08-1 (test contrôlé équipe indépendante 500+ pages 31 mars 2027) qui teste la reproductibilité. Question à Tim : cette lecture single-source vendor + pattern général corroboré est-elle acceptable comme angle info du jour, ou faut-il durcir la règle et ne jamais faire d'info du jour sur single-source vendor même avec pattern corroboré, quitte à ne traiter Otterly qu'en brève ? La règle actuelle de SKILL.md dit « une source explore peut lancer une piste mais ne suffit jamais à publier un claim » ; ici la source Otterly n'est plus explore (2e hit cumul), donc l'exception fonctionne. Mais le principe méritez d'être clarifié.

- **Q-2026-08-08-2 (non urgent, revue hebdo)** — Proposition de nouveau concept doctrine : `wiki/concepts/mesure-separee-follow-up-ai-mode.md`. Le fait 08-08 sur GSC AI Mode follow-up queries (Yes, Yes go on) documenté par Dan Taylor SALT.agency et repris par Barry Schwartz introduit un mécanisme distinct dans la doctrine [[concepts/metriques-visibilite-geo]] : sur la surface AI Mode, il faut distinguer le prompt initial de ses follow-up parce que Google Search Console les compte comme queries séparées. La méthode d'audit (identifier motifs conversationnels courts <3 mots avec yes/no/continue/source/summarise dans queries GSC) est actionnable et propre. Question à Tim : cette proposition passe-t-elle le seuil de création d'un concept doctrine séparé, ou reste-t-elle une note dans [[concepts/metriques-visibilite-geo]] ?
